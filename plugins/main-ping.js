...
let handler = async (m, { conn }) => {
    ...
    try {
        if (process.platform === 'win32') {
            ...
            diskInfo = `Usado ${(used / 1024 / 1024 / 1024).toFixed(2)} GB , ${(free / 1024 / 1024 / 1024).toFixed(2)} GB libres , de ${(total / 1024 / 1024 / 1024).toFixed(2)} GB`
        } else {
            const output = execSync('df -k --output=avail,size /').toString().split('\n')
            const [free, total] = output[1].trim().split(/\s+/).map(Number)
            const used = total - free
            diskInfo = `Usado ${(used / 1024 / 1024).toFixed(2)} GB , ${(free / 1024 / 1024).toFixed(2)} GB libres , de ${(total / 1024 / 1024).toFixed(2)} GB`
        }
    } catch {
...
┃ 🗂️ *RAM:* Usado ${usedMem} GB , ${freeMem} GB libres , de ${totalMem} GB
┃ 💽 *Espacio:* ${diskInfo}
